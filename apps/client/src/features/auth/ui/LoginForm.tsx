...
export function RegisterPlaceholder({
  ...
  return (
    <Box className="w-full space-y-4 text-left">
      ...
      <Box className="rounded-2xl border border-dashed border-border bg-muted/40 p-8 text-center space-y-3">
        <Box className="h-12 w-12 mx-auto rounded-full bg-primary/10 text-primary flex items-center justify-center">
        ...
        {onSwitchToLogin && (
          ...
          >
            {AUTH_UI_TEXT.HAVE_ACCOUNT} {AUTH_UI_TEXT.LOGIN_NOW}
          </Button>
        )}
      </Box>
    </Box>
